...
public class TETile {
    ...
    public void drawDarker(double x, double y) {
        if (filepath != null) {
            try {
                StdDraw.picture(x + 0.5, y + 0.5, filepath.substring(0, filepath.length() - 4) + "1.png", 1, 1);
                return;
            } catch (IllegalArgumentException e) {
                // Exception happens because the file can't be found. In this case, fail silently
                // and just use the character and background color for the tile.
        ...
        StdDraw.setPenColor(backgroundColor.darker());
        StdDraw.filledSquare(x + 0.5, y + 0.5, 0.5);
        StdDraw.setPenColor(textColor.darker());
        StdDraw.text(x + 0.5, y + 0.5, Character.toString(character()));
        StdDraw.enableDoubleBuffering();
    }
    public void drawEvenDarker(double x, double y) {
        if (filepath != null) {
            try {
                StdDraw.picture(x + 0.5, y + 0.5, filepath.substring(0, filepath.length() - 4) + "2.png", 1, 1);
                return;
            } catch (IllegalArgumentException e) {
                // Exception happens because the file can't be found. In this case, fail silently
                // and just use the character and background color for the tile.
        ...
        StdDraw.setPenColor(backgroundColor.darker().darker());
        StdDraw.filledSquare(x + 0.5, y + 0.5, 0.5);
        StdDraw.setPenColor(textColor.darker().darker());
        StdDraw.text(x + 0.5, y + 0.5, Character.toString(character()));
        StdDraw.enableDoubleBuffering();
    }
